...
export function buildPasswordResetEmail(params: { resetUrl: string; appName?: string }): { subject: string; html: string } {
  ...
  const bodyHtml = [
    ...
    emailParagraph(`Vous avez demande la reinitialisation de votre mot de passe ${appName}. Cliquez sur le bouton ci-dessous pour en choisir un nouveau.`),
    buildEmailButton({
      href: params.resetUrl,
      label: 'Reinitialiser mon mot de passe',
    }),
    emailMutedParagraph(`Ce lien expire dans 1 heure. Si vous n'etes pas a l'origine de cette demande, ignorez cet e-mail.`),
  ].join('\n');
